        ...
        public override Task InitSteps()
        {
            Steps.Add(new Step<string>(new()
            {
                Prompt = new SelectionPrompt<string>()
                    .Title($"Which [{Constants.SUCCESS_COLOR}]RestoreMode[/]? [{Constants.SUCCESS_COLOR}]({Options.RestoreMode})[/]")
                    .PageSize(10)
            ...
            {
                Prompt = new ConfirmationPrompt($"[{Constants.SUCCESS_COLOR}]Included[/] object types: {string.Join(";", Options.IncludedObjectTypes ?? Enumerable.Empty<string>())}\nWould you like to change them?")
                {
                    DefaultValue = false
                },
            ...
            Steps.Add(new Step<string>(new()
            {
                Prompt = new TextPrompt<string>($"Enter new [{Constants.SUCCESS_COLOR}]included[/] object types separated by semi-colon:")
                    .AllowEmpty(),
                ValueReceiver = (v) =>
                {
            ...
            Steps.Add(new Step<bool>(new()
            {
                Prompt = new ConfirmationPrompt($"[{Constants.SUCCESS_COLOR}]Excluded[/] object types: {string.Join(";", Options.ExcludedObjectTypes ?? Enumerable.Empty<string>())}\nWould you like to change them?")
                {
                    DefaultValue = false
                },
            ...
            Steps.Add(new Step<string>(new()
            {
                Prompt = new TextPrompt<string>($"Enter new [{Constants.SUCCESS_COLOR}]excluded[/] object types separated by semi-colon:")
                    .AllowEmpty(),
                ValueReceiver = (v) =>
                {
                    string[] types = v.Split(';');
