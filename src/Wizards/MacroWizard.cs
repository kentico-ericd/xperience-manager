...
namespace Xperience.Xman.Wizards
{
    ...
    public class MacroWizard : AbstractWizard<MacroOptions>
    {
        public override Task InitSteps()
        {
            ...
            {
                Prompt = new ConfirmationPrompt($"[{Constants.SUCCESS_COLOR}]Sign all[/] macros?")
                {
                    DefaultValue = true
                },
            ...
            {
                Prompt = new TextPrompt<string>($"[{Constants.SUCCESS_COLOR}]Username[/]:"),
            ...
            Steps.Add(new Step<string>(new()
            {
                Prompt = new TextPrompt<string>($"[{Constants.SUCCESS_COLOR}]Old[/] salt:"),
            ...
            Steps.Add(new Step<string>(new()
            {
                Prompt = new TextPrompt<string>($"[{Constants.SUCCESS_COLOR}]New[/] salt? Leave empty to use the salt in appsettings:")
                    .AllowEmpty(),
